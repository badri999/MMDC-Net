...
def copy_files_with_suffix(files: List[Tuple[Path, str]], output_dir: Path, logger) -> int:
    """
    Copy files to output directory with subfolder name as suffix
...
def download_results(base_results_path: str, filename: str, split: str, output_dir: str = "results to download"):
    ...
    try:
        ...
        copied_count = copy_files_with_suffix(matching_files, output_path, logger)
        
        logger.info(f"Successfully copied {copied_count} files to '{output_path}'")
    ...
    for filename in filenames:
        try:
            ...
            if matching_files:
                ...
                logger.info(f"Copied {copied_count} files for '{filename}'")
            else:
...
def main():
    parser = argparse.ArgumentParser(description='Download specific results files with proper naming')
    parser.add_argument(
        '--base_path',
        type=str,
        default='/home/badri/Hard Disk Project/neuralnets/depth_fusion/three_branch/model/results/squeezeunet_fusion_branches_2_5/predictions/',
        help='Base path to results directory'
    )
    parser.add_argument(
        '--filename',
        type=str,
        help='Filename to search for (without extension), e.g., "1_sample_a"'
    )
    parser.add_argument(
        '--split',
        type=str,
    ...
    parser.add_argument(
        '--output_dir',
        type=str,
        default='results to download',
        help='Output directory name (default: "results to download")'
    )
    parser.add_argument(
        '--batch',
        type=str,
        nargs='+',
        help='Download multiple files at once, e.g., --batch file1 file2 file3'
    )
    parser.add_argument(
        '--list',
        action='store_true',
